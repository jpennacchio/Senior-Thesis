...
from regression_formulas import ols, two_stage_least_squares
...
def Residual_Bootstrap_2SLS_NoSelection(x, y, d, z, num_bootstrap_samples = 1000, nproc=8, tol=1e-5, method = 0):
    ...
    if(method==0):
        ...
        print(instruments)
        z = z[:,instruments]
    ###Otherwise, use pre-specified instrument list.####
    ...
    coefficients = ols(d, [], xz, return_type = 2, dataframe=False, stage1 = True)
    ####STEP 1B:#########
   ...
   ###Combine instruments and controls into a dataframe. The coefficients for the controls go first per omp and lasso setup. 
    ...
    ###Run 2SLS here---no variable selection. We weeded out instruments up front.###
    treatment_coefficients_2SLS= Parallel(n_jobs = nproc, verbose = 150)(delayed(two_stage_least_squares)(adjusted_y_values[:,i], adjusted_d_values[:,i], x, z, dataframe = False) \
                                 for i in range(0,num_bootstrap_samples)) 
